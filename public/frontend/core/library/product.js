(function($) {
	...
	HT.setUpVariantPrice = (res) => {
		console.log(res.variantPrice.html)
		$('.popup-product .price').html(res.variantPrice.html)
		
	}

	HT.setupVariantName = (res) => {
		let sku = res.variant['sku']; // Lấy giá trị của trường 'sku' từ đối tượng 'variant' trong đối số 'res'
		// let quantity = res.variant;
    	$('.masp strong').html(sku); // In giá trị của trường 'sku' vào phần tử có thẻ <strong> trong phần tử có class 'spec-row'
		console.log(res.variantPrice.html);
		let productName = $('.productName').val()
		let productVariantName = productName + ' ' + res.variant.languages[0].pivot.name
		let tietkiem = res.variantPrice.price - res.variantPrice.priceSale
		$('.product-main-title span').html(productVariantName)
		$('.price-save strong').html(tietkiem.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' }).replace(/,/g, '.'))
		$('.price-save span').html("-" + res.variantPrice.percent + "%")
